...
A multi-purpose Telegram bot built with Python that seamlessly integrates **Daily Task Reporting** with a powerful **AI Chat Assistant** powered by Gemini. 
...
- **All-in-One AI Chatbot**: Talk to the bot normally to get instant AI responses powered by Google's Gemini models. It automatically detects the difference between a task update and a chat message.
...
- `python-telegram-bot`
- `python-dotenv`
- `httpx` (for async Gemini API calls)
...
By default the bot uses Google's extremely fast model: `gemini-2.5-flash`.
You can override it with `GEMINI_MODEL` (and optionally `GEMINI_MAX_TOKENS`, `GEMINI_TEMPERATURE`).

## Deploy on Your Own Server (Docker, 24/7)

1. Install Docker on your server.
2. Make sure `BOT_TOKEN` and `GEMINI_API_KEY` are set in `.env`.
3. Build and start:
...
2. **Chatting with AI:**
   Just talk to the bot! If your message doesn't format like a task update (i.e. doesn't end in a number), the bot will forward your query to the Gemini AI and reply instantly.
